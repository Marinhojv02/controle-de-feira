    ...
    async create(req: Request, res: Response) {
        try {
            ...
            if (!req.body.shopping_list_items) {
                const shopping_list_items = await ShoppingListUsecase.generateDefaultShoppingList(req.body.house_id, savedShoppingList)
                res.status(201).json({
                    status: "Created!",
                    message: "Successfully created default shopping list!",
                });
            }
