...
interface IHouseProducts {
    ...
    retrieveByHouseId(house_id: number): Promise<HouseProduct[]>;
    retrieveByProductId(product_id: number): Promise<HouseProduct[]>;
...
export class HouseProductsRepo implements IHouseProducts {
    ...
    async retrieveByHouseId(house_id: number): Promise<HouseProduct[]> {
        try {
    ...
    async retrieveByProductId(product_id: number): Promise<HouseProduct[]> {
        try {
            const new_houseProduct = await HouseProduct.findAll({
                where: {
    ...
    async retrieveByHouseAndProductId(house_id: number, product_id: number): Promise<HouseProduct | null> {
        try {
            const new_houseProduct = await HouseProduct.findOne({
                where: {
                    house_id: house_id,
