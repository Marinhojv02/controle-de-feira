...
class HouseProductUsecase {
    async create(product_id: number, house_id: number, quantity_in_stock: number, reorder_point: number, recommended_quantity: number) {
        ...
        return await new HouseProductsRepo().save(new_houseProduct);
    }
    async findById(id: number) {
        return await new HouseProductsRepo().retrieveById(id);
    }
    async findAll() {
